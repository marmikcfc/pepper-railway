...
// --- Trigger gating with isDM flag ---

describe('trigger gating (isDM interaction)', () => {
  // Replicates the exact logic from processGroupMessages and startMessageLoop:
  //   if (!isDM && group.requiresTrigger !== false) { check trigger }
  function shouldRequireTrigger(
    isDM: boolean,
    requiresTrigger: boolean | undefined,
  ): boolean {
    return !isDM && requiresTrigger !== false;
  }
  ...
  function shouldProcess(
    ...
    if (!shouldRequireTrigger(isDM, requiresTrigger)) return true;
    return messages.some((m) => TRIGGER_PATTERN.test(m.content.trim()));
  }

  it('DM always processes (no trigger needed)', () => {
    const msgs = [makeMsg({ content: 'hello no trigger' })];
    expect(shouldProcess(true, undefined, msgs)).toBe(true);
  });

  it('DM processes even with requiresTrigger=true', () => {
    const msgs = [makeMsg({ content: 'hello no trigger' })];
